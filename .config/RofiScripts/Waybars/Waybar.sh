...
chosen=$(printf "Balken\nSchwebend\nInsel\nAngedockt" | rofi -dmenu -i -config '~/.config/RofiScripts/Waybars/WB.rasi')
...
case "$chosen" in
   ...
   "Angedockt") ~/.config/RofiScripts/Waybars/WaybarThemes/docked/docked.sh & ~/.config/nwg-dock-hyprland/launch.sh ;;
   *) exit 1 ;;
esac
